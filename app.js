...
function loadPreferences(){try{return JSON.parse(localStorage.getItem(PREFERENCES_STORAGE))||{};}catch{return {}}}
function savePreferences(){localStorage.setItem(PREFERENCES_STORAGE,JSON.stringify({autoHideAmounts}));}
function distribute(){
...
function render(){
  ...
  const expandedScenarios=new Set([...document.querySelectorAll('#scenarios .scenario details')].flatMap((details,index)=>details.open?[index]:[]));
  $('#scenarioEmpty').hidden=state.scenarios.length>0;$('#scenarios').innerHTML='';state.scenarios.forEach((s,index)=>{renderScenario(s,index);$('#scenarios').lastElementChild.querySelector('details').open=expandedScenarios.has(index)}); save();updateStickyHeader();
}
...
$('#autoHideAmounts').onchange=e=>{autoHideAmounts=e.target.checked;savePreferences()};
